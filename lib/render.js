...
function insertReadmeTable(readmeFilePath, sections, config) {
  ...
  lines.forEach((line, i) => {
    ...
    const match = line.match(
      new RegExp(`^(##+) ${config.regexp.paramsSectionTitle}`),
  ...
  if (paramsSectionLimits.length === 1) {
    ...
    const endParamsSectionRegExp = new RegExp(
      "(?!.*\\|).*\\S(?<!\\|.*)(?<!#.*)",
    ); // Match non empty or with non table format lines
    lines
      .slice(paramsSectionLimits[0], paramsSectionLimits[1])
      .reverse()
      .forEach((line, i) => {
        if (
          !lastTableLikeLineFound &&
          line &&
          !line.match(endParamsSectionRegExp)
        ) {
          ...
          console.log(
            `INFO: Last parameter table line found at line: ${paramsSectionLimits[1]}`,
          );
        }
      });
  ...
  lines.splice(
    paramsSectionLimits[0],
    paramsSectionLimits[1] - paramsSectionLimits[0] + 1,
  );
...
function renderOpenAPISchema(schemaFilePath, parametersList, config) {
  ...
  const nilValues = paramsList
    .filter(
      (p) =>
        p.value === "nil" &&
        !utils.containsModifier(p, config.modifiers.nullable),
    )
    .map((p) => p.name);
  if (nilValues.length > 0) {
    throw new Error(
      `Invalid type 'nil' for the following values: ${nilValues.join(", ")}`,
    );
  }
  ...
  paramsList.forEach((p) => {
    if (p.nullable && p.value === "null") {
      console.log("Adding array type with null");
      p.type = [p.type, "null"];
    }
  });
  ...
  paramsList = paramsList.filter(
    (p) => !utils.containsModifier(p, config.modifiers.object),
  );
