...
function applyModifiers(param, config) {
  ...
  const index =
    param.modifiers.findIndex((m) => m === config.modifiers.nullable) + 1;
    ...
    param.modifiers.forEach((modifier) => {
      switch (modifier) {
        ...
        case `${config.modifiers.nullable}`:
          ...
          break;
        case modifier.match(new RegExp(`${config.modifiers.default}:.*`))
          ?.input: {
        ...
        default:
          throw new Error(
            `Unknown modifier: ${modifier} for parameter ${param.name}`,
          );
      }
    });
  }
